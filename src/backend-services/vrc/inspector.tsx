...
const TextboxInspector: FC = () => {
  const state = useSnapshot(window.API.state.services.vrc.data.textbox);
  const up = <K extends keyof VRC_State["textbox"]>(key: K, v: VRC_State["textbox"][K]) => window.API.patchService("vrc", s => s.data.textbox[key] = v);
  return <>
...
const KillFrenzyInspector: FC = () => {
  const state = useSnapshot(window.API.state.services.vrc.data.killfrenzy);
...
const Inspector_VRC: FC = () => {
  const state: VRC_State = useSnapshot(window.API.state.services.vrc.data);
